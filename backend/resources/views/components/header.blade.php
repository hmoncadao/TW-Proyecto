<header class="bg-white dark:bg-slate-900 border-b border-[#C4C7CF] dark:border-slate-700 fixed top-0 left-0 right-0 z-50">
    ...
    <div class="flex justify-between items-center h-16 w-full px-4 sm:px-6 max-w-[1280px] mx-auto gap-3">
        ...
        <div class="flex items-center shrink-0">
            <a href="/" class="text-lg sm:text-xl md:text-2xl font-bold text-[#1B365D] dark:text-blue-400 tracking-tight whitespace-nowrap">
                GovConnect
            </a>
        </div>
        ...
        <div class="flex items-center gap-2 sm:gap-3 ml-auto min-w-0">
            ...
            @auth
                ...
                <div class="flex items-center gap-2 text-center min-w-0">
                    ...
                    <span class="text-sm sm:text-base md:text-lg font-semibold text-slate-700 dark:text-slate-300 truncate max-w-[120px] sm:max-w-[170px] md:max-w-none">
                        {{ auth()->user()->name }}
                    </span>

                    @if(auth()->user()->isAdmin())
                        <span class="text-xs sm:text-sm font-bold bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 px-2 sm:px-3 py-1 rounded-full whitespace-nowrap">
                            Admin
                        </span>
                    @else
                        <span class="text-xs sm:text-sm font-bold bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 px-2 sm:px-3 py-1 rounded-full whitespace-nowrap">
                            Usuario
                        </span>
                    @endif
